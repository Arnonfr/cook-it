...
const IngredientImageService_1 = require("../services/IngredientImageService");
...
const imageService = new IngredientImageService_1.IngredientImageService();
...
router.get('/steps/images', async (req, res) => {
    try {
        const textsStr = req.query.texts;
        if (!textsStr) {
...
router.get('/catalog', async (_req, res) => {
    try {
        ...
        const catalogResults = mockRecipes_1.mockRecipes.map(r => (0, recipeTransforms_1.mockRecipeToSearchResult)(r));
        let dbRecipes = [];
        try {
        ...
        if (userId) {
            try {
                ...
                for (const r of local)
                    r.isMine = savedUrls.has(r.sourceUrl);
                for (const r of dedupedWeb)
                    r.isMine = savedUrls.has(r.sourceUrl);
            }
            catch { }
        }
...
router.get('/parse', async (req, res) => {
    try {
        ...
        try {
            ...
            if (recipe.steps) {
                const stepTexts = recipe.steps.map((s) => s.text || s);
                // Trigger lazily
